...
import { doanhThuTheoNam } from "../../../services/adminService";
...
export default function DoanhThuNamScreen(){
  const [nam, setNam] = useState(String(new Date().getFullYear()));
  const [data, setData] = useState<any>(null);

  const onLoad = async () => {
    const res = await doanhThuTheoNam(Number(nam));
    setData(res.data);
  };

  const mapToBar = () => {
    const items = data?.chiTietTheoThang || [];
    const labels = items.map((x:any) => `T${x.thang}`);
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ paddingBottom: 16 }}>
      <Text style={s.title}>Doanh thu theo năm</Text>

      <View style={s.inputWrap}>
        <TextInput value={nam} onChangeText={setNam} keyboardType="number-pad" placeholder="Năm" placeholderTextColor={Colors.textMuted} style={s.input}/>
      </View>
      ...
      {data && (
        <>
          ...
          <View style={s.card}>
            <Text style={s.cardTitle}>Thực thu theo tháng</Text>
           <BarChart
  data={mapToBar()}
